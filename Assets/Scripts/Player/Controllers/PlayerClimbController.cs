...
public class PlayerClimbController : MonoBehaviour
{
    ...
    [Space(20)]
    [Header("====Debugs====")]
    [SerializeField] float _detectedWallHeight;
    [SerializeField] bool _isVault;
    ...
    [SerializeField] Vector3 _finalClimbPosition;
    [SerializeField] Vector3 _startClimbPosition;
    ...
    public bool CanClimbWall()
    {
        ...
        for(int i=0; i<_rayCount; i++)
        {
            ...
            if(Physics.Raycast(transform.position + new Vector3(0, i / _rayDencity, 0), transform.forward, out detectRayHit, _wallDetectionRange, _climbMask))
            {
                break;
            }
            missedRaysCount++;
        ...
        isWallDetected = missedRaysCount < _rayCount;

        if (!isWallDetected) return false;
    ...
    public bool GetIsVault()
    {
        return _isVault;
    }
    public float GetClimbHeight()
    {
        return _detectedWallHeight;
    }
    public Vector3 GetFinalClimbPosition()
    {
        return _finalClimbPosition;
    }
    public Vector3 GetStartClimbPosition()
    {
        return _startClimbPosition;
    }
